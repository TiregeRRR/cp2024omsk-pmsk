# Запуск локальной для разработки

Пока тут .devcontainer, по идее надо в vscode открыть только папку front и запусить devcontainer
либо просто локально

`npm run dev`

для домена:

1) создать аккаунт на ngrok
2) если хочется постоянный домен - https://dashboard.ngrok.com/cloud-edge/domains 
`ngrok http --domain=tetra-trusting-urgently.ngrok-free.app 4444             `

# Запуск продашкен билда

docker build .  --build-arg VITE_BFF_HOST=https://fitting-oriole-primarily.ngrok-free.app --tag front           
docker run -p 8000:8000 --rm -it front
