...
def generic_transcription(client_url):
    ...
    assert wait_for_task_completion(identifier), f"Task with identifier {identifier} did not complete within the expected time."

    task_result = client.get(f"/task/{identifier}")
    seg_0_text = task_result.json()["result"]["segments"][0]["text"]
    assert seg_0_text.startswith(TRANSCRIPT_RESULT_1) or seg_0_text.startswith(TRANSCRIPT_RESULT_2)

    return task_result.json()["result"]


def align(transcript_file):
    with open(transcript_file, "rb") as transcript_file, open(AUDIO_FILE, "rb") as audio_file:
        response = client.post(
            "/service/align",
            files={
...
def combine(aligned_transcript_file, diarazition_file):
    with open(aligned_transcript_file, "rb") as transcript_file, open(diarazition_file, "rb") as diarization_result:
        files = {
...
def test_combine():
    result = combine("test_files/aligned_transcript.json", "test_files/diarazition.json")

    assert result["segments"][0]["text"].startswith(TRANSCRIPT_RESULT_1) or result["segments"][0]["text"].startswith(
        TRANSCRIPT_RESULT_2
    )
...
def test_speech_to_text_url():
    ...
    response = client.post(
        ...
        data={"url": "https://github.com/tijszwinkels/whisperX-api/raw/main/audio_en.mp3"},
    )
    assert response.status_code == 200
    assert "Task queued" in response.json()["message"]
